...
-- NOTE: This runs as a trigger on public.users (allowed). The SECURITY DEFINER
-- function updates auth.users.raw_app_meta_data so the JWT carries role/tenant_id.
-- If auth schema is locked, the function silently succeeds (claims won't update
-- until the user re-logs and the setup_new_user RPC handles it as a fallback).
-- ============================================================================
...
BEGIN
  ...
  EXCEPTION WHEN insufficient_privilege THEN
    -- Silently skip if auth schema is restricted; app handles it via setup_new_user.
    NULL;
  END;
  RETURN NEW;
END;
